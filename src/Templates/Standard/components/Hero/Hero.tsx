...
const MarketingHero: FC<{ isMobile: boolean; user?: User; siteConfig: PageSiteConfig }> = ({ isMobile, user, siteConfig }) => {
  const { globalState } = useAppContext();
  const { heroSection } = siteConfig;

  let bannerAlign =
    isMobile && heroSection?.banner?.position !== "background" ? "bottom" : (heroSection?.banner?.position as bannerAlignment);
