...
export const getEnrolledListByCourse = async (
  ...
  let result = await prisma.$queryRaw<
    any[]
  >` SELECT Count(o.id) as totalEnrolled,  usr.name as studentName, Count(cp.resourceId) as watchedLessons,MAX(cp.createdAt) as lastActivity, COUNT(re.resourceId) as TotalLessons,o.updatedAt as dateJoined From \`Order\` as o
   ...
   UNION
    SELECT Count(o.id) as totalEnrolled,  usr.name as StudentName,Count(cp.resourceId) as watchedLessons,MAX(cp.createdAt) as lastActivity, COUNT(re.resourceId) as TotalLessons,o.updatedAt as dateJoined From LearningPathCourses as lpc
  ...
  if (result.length > 0) {
    const courseProgress = result.map((cp: any) => {
      return {
        ...
        lastActivity: cp.lastActivity ? generateDayAndYear(cp.lastActivity) : "",
    ...
    return new APIResponse(true, 200, "Enrolled users list has been fetched", {
      ...
      total: Number(result[0].totalEnrolled),
    });
  } else {
