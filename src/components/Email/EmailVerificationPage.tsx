import { IEmailVerificationConfig, IWelcomeEmailConfig } from "@/lib/emailConfig";
import { capitalizeFirst, capsToPascalCase } from "@/lib/utils";
...
interface IProps {
  configData: IEmailVerificationConfig;
}

const EmailVerificationPage = ({ configData }: IProps) => {
        ...
        <Body className="bg-[#f5f5f5] my-auto mx-auto font-sans ">
          <Container className="border border-solid border-[#eaeaea] rounded my-[40px] mx-auto    max-w-[465px]">
            ...
            <Section className="px-[20px]">
              <Text className="text-black text-[20px] leading-[20px]">
                Your {configData.mode} request to {capsToPascalCase(`${site.brand?.name}`)}
              </Text>
              <Text className="text-[#888] text-[14px] leading-[20px]">
                Click the button below to{" "}
                {configData.mode == "login" ? "log into" : "confirm your email verification in"}{" "}
                {capsToPascalCase(`${site.brand?.name}`)}. Your link expires in 1 hour.
              </Text>
              <Button
                href={configData.url}
                className={`bg-[${site.brand?.brandColor}] px-5 py-2 text-white text-left text-[12px]  rounded`}
              >
                {capitalizeFirst(configData.mode)}
              </Button>
